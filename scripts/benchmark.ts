...
async function distribute(count: number, ethers) {
  ...
  await ensureFinished(mockUSDCContract.mint(masterAcc.address, totalUSDC.toFixed()))
  await ensureFinished(mockUSDCContract.connect(masterAcc).approve(disperseContract.address, totalUSDC.toFixed()))
  // create traders
  ...
  for (let i = 0; i < traders.length; i += BATCH_SIZE) {
    ...
    await ensureFinished(disperseContract.connect(masterAcc).disperseToken(
      mockUSDCContract.address,
      batch.map(x => x.address),
      batch.map(x => USDC_PER_TRADER.toFixed())
    ))
    // ETH: distribute 1 eth to traders
    await ensureFinished(disperseContract.connect(masterAcc).disperseEther(
      batch.map(x => x.address),
  ...
  console.log("End distribute " + endTime + " spend time " + (endTime-startTime)/1000 + "s")
}
...
async function setup(ethers, deployer, accounts) {
  ...
  await ensureFinished(mockUSDCContract.connect(masterAcc).approve(latestLiquidityPoolContract.address, "25000000" + "000000"))
  await ensureFinished(latestLiquidityPoolContract.connect(masterAcc).addLiquidity(toWei("25000000")))
...
async function tradeBenchmark(deployer) {
  ...
  const ops = async (x) => {
    return latestLiquidityPoolContract.connect(x).trade(
      0,
      x.address,
      toWei("10"),
      toWei("510"),
  ...
  console.log(
    "End Sent", (end1Time - startTime) / 1000,
    'tps', traders.length / (end1Time - startTime) * 1000,
    " of ", traders.length, "trader"
  )
  const receipts = await Promise.all(txs.map(x => x.wait()))
  const end2Time = Date.now()
  console.log(
    'End trade', (end2Time - startTime) / 1000,
    'tps', traders.length / (end2Time - startTime) * 1000,
    ' of ', traders.length, 'trader'
  )
  ...
  poolGetterContract = await deployer.getContractAt("Getter", latestLiquidityPoolContract.address);
  for (let trader of traders) {
    const {position, isMarginSafe} = await poolGetterContract.getMarginAccount(0, trader.address)
    assert.equal(isMarginSafe, true)
    assert.equal(position, TRADER_LEVERAGE.toFixed(), `trader ${trader.address}'s position ${position}`)
  }
}
...
async function preTrade() {
  ...
  const txOps = async (trader) => {
    await ensureFinished(mockUSDCContract.connect(trader).approve(latestLiquidityPoolContract.address, USDC_PER_TRADER.toFixed()))
    // align createPerpetual 10 leverage
    await ensureFinished(latestLiquidityPoolContract.connect(trader).setTargetLeverage(0, trader.address, TRADER_LEVERAGE.toFixed()))
  }
  const txs = traders.map(x => txOps(x))
  await Promise.all(txs)
  let endTime = Date.now()
  console.log("End preTrade " + endTime + " spend time " + (endTime-beginTime)/1000 + "s")
}
